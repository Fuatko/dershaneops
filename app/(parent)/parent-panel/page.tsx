...
export default function ParentPanelPage() {
  ...
  function getWeekDaysLocal(start: Date) {
    return Array.from({ length: 7 }, (_, i) =>
      new Date(start.getFullYear(), start.getMonth(), start.getDate() + i)
    )
  }
  ...
  const thisWeekDays = getWeekDaysLocal(getMonday(new Date()))
  const thisWeekStart = localDate(thisWeekDays[0])
  const thisWeekEnd = localDate(thisWeekDays[6])
  const thisWeekCalendar = calendar.filter(c => c.calendar_date >= thisWeekStart && c.calendar_date <= thisWeekEnd)
          ...
          {activeTab === 'dashboard' && (
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'10px', marginBottom:'14px' }}>
                {[
                  ...
                  <div key={m.label} style={{ background:P.white, borderRadius:'10px', padding:'13px', border:'1px solid '+P.border, display:'flex', alignItems:'center', gap:'10px' }}>
                    <div style={{ width:'36px', height:'36px', borderRadius:'9px', background:m.bg, border:'1px solid '+m.border, display:'flex', alignItems:'center', justifyContent:'center', color:m.color, flexShrink:0 }}><m.Icon /></div>
                    <div>
              ...
              {getCalForDate(todayStr).length > 0 && (
                <div style={{ background:P.white, borderRadius:'12px', overflow:'hidden', marginBottom:'14px', border:'1px solid '+P.border }}>
                  <div style={{ padding:'12px 16px', borderBottom:'1px solid '+P.border, fontSize:'12px', fontWeight:700, color:P.text }}>Bugünün Çalışma Programı</div>
            ...
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'12px' }}>
                ...
                <div style={{ display:'flex', gap:'2px', background:P.slateLight, borderRadius:'7px', padding:'3px' }}>
                  {(['week','month'] as const).map(v => (
                    <button key={v} onClick={() => setCalendarView(v)} style={{ padding:'5px 12px', borderRadius:'5px', border:'none', background:calendarView===v?P.white:'transparent', color:calendarView===v?P.primary:P.muted, fontSize:'11px', fontWeight:calendarView===v?700:500, cursor:'pointer' }}>
                    ...
                    {weekDays.map((day,i) => {
                      ...
                      return (
                        <button key={i} onClick={() => setSelectedDate(dateStr)}
                          style={{ padding:'7px 3px', borderRadius:'8px', border:'1.5px solid', borderColor:isSelected?P.primary:holiday?holiday.color:isToday?P.primaryBorder:P.border, background:isSelected?P.primary:holiday?holiday.color+'25':isToday?P.primaryLight:isWeekend?P.slateLight:P.white, cursor:'pointer', textAlign:'center' }}>
                          <div style={{ fontSize:'8px', color:isSelected?'rgba(255,255,255,0.6)':P.muted, marginBottom:'2px' }}>{DAYS_SHORT[i]}</div>
                          <div style={{ fontSize:'13px', fontWeight:700, color:isSelected?'#fff':holiday&&!isToday?holiday.color:isToday?P.primary:P.text }}>{day.getDate()}</div>
              ...
              <div style={{ background:P.white, borderRadius:'10px', overflow:'hidden', border:'1px solid '+P.border, marginBottom:'12px' }}>
                <div style={{ padding:'11px 14px', borderBottom:'1px solid '+P.border, background:P.slateLight }}>
                  ...
                  {selectedNote && (
                    <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 8px', borderRadius:'4px', background:evalCfg[selectedNote.evaluation]?.bg, color:evalCfg[selectedNote.evaluation]?.color, border:'1px solid '+evalCfg[selectedNote.evaluation]?.border, display:'inline-block', marginTop:'6px', textTransform:'uppercase', letterSpacing:'0.3px' }}>
                ...
                {selectedNote?.note && (
                  <div style={{ padding:'12px 14px', borderBottom:'1px solid '+P.border, background:evalCfg[selectedNote.evaluation]?.bg }}>
                    <div style={{ fontSize:'10px', color:P.muted, marginBottom:'4px' }}>Öğretmen Notu — {selectedNote.profiles?.full_name}</div>
                  ...
                  return (
                    <div key={item.id} style={{ padding:'11px 14px', borderBottom:i<selectedItems.length-1?'1px solid '+P.border:'none', display:'flex', alignItems:'center', gap:'10px', background:isDone?'#F0FDF4':P.white }}>
                      <div style={{ width:'34px', height:'34px', borderRadius:'8px', background:isDone?P.greenLight:P.slateLight, border:'1px solid '+(isDone?P.greenBorder:P.border), display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                        {isDone ? <span style={{ color:P.green }}><Icon.check /></span> : <><div style={{ fontSize:'12px', fontWeight:700, color:P.slate }}>{item.duration_minutes}</div><div style={{ fontSize:'8px', color:P.muted }}>dk</div></>}
                      </div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:'12.5px', fontWeight:600, color:isDone?P.muted:P.text, textDecoration:isDone?'line-through':'none', marginBottom:'2px' }}>{item.title}</div>
          ...
          {activeTab === 'performance' && (
            <div>
              ...
              {topicPerf.length===0 ? (
                ...
                return (
                  <div key={t.id} style={{ background:P.white, borderRadius:'10px', padding:'12px 14px', marginBottom:'6px', border:'1px solid '+P.border }}>
