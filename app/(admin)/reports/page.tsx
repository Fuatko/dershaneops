  ...
  function exportCSV() {
    ...
    if (activeReport === 'performance') {
      ...
      rows = reportData.map((r: any) =>
        `"${r.profiles?.full_name}","${r.profiles?.grade_level}. Sınıf","${r.subjects?.name}","${r.topics?.name ?? 'Genel'}","${Math.round(r.accuracy_rate)}","${r.total_questions}","${r.correct_count}","${r.wrong_count}"`
      )
    }
    if (activeReport === 'homework') {
      headers = 'Öğrenci,Sınıf,Test,Kitap,Durum'
      rows = reportData.map((r: any) =>
        `"${r.profiles?.full_name}","${r.profiles?.grade_level}. Sınıf","${r.tests?.name}","${r.tests?.chapters?.books?.name ?? ''}","${r.status === 'completed' ? 'Tamamlandı' : 'Bekliyor'}"`
      )
    }
    if (activeReport === 'attendance') {
      headers = 'Öğrenci,Sınıf,Tarih,Görev,Ders,Durum,Süre'
      rows = reportData.map((r: any) =>
        `"${r.profiles?.full_name}","${r.profiles?.grade_level}. Sınıf","${r.calendar_date}","${r.title}","${r.subjects?.name ?? ''}","${r.status === 'completed' ? 'Tamamlandı' : 'Bekliyor'}","${r.duration_minutes} dk"`
      )
    }
    if (activeReport === 'risk') {
      headers = 'Öğrenci,Sınıf,Risk Skoru,Genel Başarı'
      rows = reportData.map((r: any) =>
        `"${r.full_name}","${r.grade_level}. Sınıf","${r.risk_score}","${r.overall_rate}%"`
      )
    }
    ...
    a.href = url
    a.download = `dershaneops_${activeReport}_${localDate()}.csv`
    a.click()
  ...
  return (
    <div style={{ padding:'20px 16px', maxWidth:'1200px', fontFamily:'-apple-system, BlinkMacSystemFont, "Inter", sans-serif' }}>
      ...
      <div style={{ background:'#fff', borderRadius:'10px', padding:'14px', marginBottom:'14px', border:'1px solid #E2E8F0' }}>
        <div style={{ fontSize:'11px', fontWeight:600, color:'#475569', textTransform:'uppercase', letterSpacing:'0.4px', marginBottom:'10px' }}>Filtreler</div>

        {/* Sınıf */}
        <div style={{ marginBottom:'10px' }}>
          <div style={{ fontSize:'11px', color:'#94A3B8', marginBottom:'6px' }}>Sınıf</div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:'5px' }}>
            <button onClick={() => { setFilterClass(''); setFilterStudent('') }}
            ...
            {grades.map(g => (
              <button key={g} onClick={() => { setFilterClass(String(g)); setFilterStudent('') }}
                    ...
                    return (
                      <tr key={i} style={{ borderBottom:'1px solid #F8FAFC' }}>
                        ...
                        <td style={{ padding:'9px 12px' }}>
                          <span style={{ padding:'2px 8px', borderRadius:'10px', background:bg, color, fontWeight:700, fontSize:'11px' }}>%{rate}</span>
                        </td>
