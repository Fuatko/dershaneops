      ...
      <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #D5DFF0', padding: '24px' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: '14px' }}>
            ...
            <input value={form.full_name} onChange={e => setForm(p => ({ ...p, full_name: e.target.value }))} placeholder="Ayşe Yılmaz" style={inp} required />
          </div>
          <div style={{ marginBottom: '14px' }}>
            <label style={lbl}>E-posta *</label>
            <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="ayse@email.com" style={inp} required />
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <a href="/students" style={{ padding: '11px 18px', borderRadius: '9px', background: '#F0F4F9', color: '#4A6080', fontSize: '13px', fontWeight: 600, textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
              İptal
            </a>
          </div>
        </form>
