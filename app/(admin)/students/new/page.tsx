...
const GRADES = [
  { value:'1', label:'1. Sinif (Ilkokul)' },
  { value:'2', label:'2. Sinif (Ilkokul)' },
  { value:'3', label:'3. Sinif (Ilkokul)' },
  { value:'4', label:'4. Sinif (Ilkokul)' },
  { value:'5', label:'5. Sinif (Ortaokul)' },
  { value:'6', label:'6. Sinif (Ortaokul)' },
  { value:'7', label:'7. Sinif (Ortaokul)' },
  { value:'8', label:'8. Sinif (Ortaokul)' },
  { value:'9', label:'9. Sinif (Lise)' },
  { value:'10', label:'10. Sinif (Lise)' },
  { value:'11', label:'11. Sinif (Lise)' },
  { value:'12', label:'12. Sinif (Lise)' },
  { value:'13', label:'Hazirlik' },
]
...
export default function NewStudentPage() {
  ...
  const lbl = { display:'block', fontSize:'11.5px', fontWeight:600, color:'#4A6080', marginBottom:'5px' }
  ...
  async function handleSubmit() {
    ...
    if (!form.grade_level) { setError('Sinif seviyesi seciniz.'); return }
  ...
  return (
    <div style={{ padding:'28px', maxWidth:'620px', fontFamily:'sans-serif' }}>
      <div style={{ marginBottom:'24px', display:'flex', alignItems:'center', gap:'10px' }}>
        <a href="/students" style={{ fontSize:'12.5px', color:'#7A8FA8', textDecoration:'none' }}>Ogrenciler</a>
        <span style={{ color:'#D5DFF0' }}>/</span>
        <span style={{ fontSize:'12.5px', color:'#1B3A6B', fontWeight:600 }}>Yeni Ogrenci</span>
      </div>
      ...
      <div style={{ background:'#EEF3FB', border:'1px solid #BFDBFE', borderRadius:'10px', padding:'12px 16px', marginBottom:'20px' }}>
        <div style={{ fontSize:'12.5px', color:'#1B3A6B', lineHeight:1.6 }}>
          Ogrenci e-posta adresine davet maili gonderilecek.
        </div>
      </div>

      <div style={{ background:'#fff', borderRadius:'12px', border:'1px solid #D5DFF0', padding:'24px' }}>
        <div style={{ fontSize:'15px', fontWeight:700, color:'#1B3A6B', marginBottom:'20px' }}>Ogrenci Bilgileri</div>

        <div style={{ marginBottom:'14px' }}>
          <label style={lbl}>Ad Soyad *</label>
          <input value={form.full_name} onChange={e => setForm(p => ({ ...p, full_name:e.target.value }))} placeholder="Ayse Yilmaz" style={inp} />
        </div>

        <div style={{ marginBottom:'14px' }}>
          <label style={lbl}>E-posta *</label>
          <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email:e.target.value }))} placeholder="ayse@email.com" style={inp} />
        </div>
        ...
        <div style={{ fontSize:'12px', fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'14px' }}>Akademik Bilgiler</div>

        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'14px' }}>
          <div>
            <label style={lbl}>Sinif Seviyesi *</label>
          ...
          <div>
            <label style={lbl}>Sube (A-Z)</label>
            <select value={form.branch} onChange={e => setForm(p => ({ ...p, branch:e.target.value }))} style={inp}>
              <option value="">Secin...</option>
              {BRANCHES.map(b => <option key={b} value={b}>{b} Subesi</option>)}
            </select>
          </div>
        </div>
        ...
        {form.grade_level && form.branch && (
          <div style={{ background:'#DCFCE7', border:'1px solid #86EFAC', borderRadius:'8px', padding:'8px 14px', marginBottom:'14px', fontSize:'12px', color:'#14532D', fontWeight:600 }}>
            Sinif: {form.grade_level}-{form.branch}
          </div>
        )}

        <div style={{ marginBottom:'20px' }}>
          <label style={lbl}>Devam Ettigi Okul</label>
          {schools.length > 0 ? (
        ...
        {success && (
          <div style={{ background:'#EAF4EE', border:'1px solid #A7D9B8', borderRadius:'8px', padding:'10px 14px', marginBottom:'14px', fontSize:'12.5px', color:'#2E7D52', fontWeight:600 }}>
            Davet maili gonderildi! <a href="/students" style={{ color:'#1B3A6B' }}>Listeye don</a>
          </div>
        )}
