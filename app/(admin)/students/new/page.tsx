cat > app/\(admin\)/students/new/page.tsx << 'ENDOFFILE'
'use client'
    ...
    <div style={{ padding:'28px', maxWidth:'620px', fontFamily:'-apple-system,BlinkMacSystemFont,sans-serif' }}>
      ...
      <div style={{ background:'#EEF3FB', border:'1px solid #BFDBFE', borderRadius:'10px', padding:'12px 16px', marginBottom:'20px' }}>
        <div style={{ fontSize:'12.5px', color:'#1B3A6B', lineHeight:1.6 }}>
          Öğrenci e-posta adresine <strong>davet maili</strong> gönderilecek.
        </div>
      </div>
      ...
      <div style={{ background:'#fff', borderRadius:'12px', border:'1px solid #D5DFF0', padding:'24px' }}>
        ...
        <div style={{ marginBottom:'14px' }}>
          ...
          <input value={form.full_name} onChange={e => setForm(p => ({ ...p, full_name:e.target.value }))} placeholder="Ayşe Yılmaz" style={inp} />
        </div>
        ...
        <div style={{ marginBottom:'20px' }}>
          ...
          {schools.length > 0 ? (
            ...
            <input value={form.school_id} onChange={e => setForm(p => ({ ...p, school_id:e.target.value }))} placeholder="Okul adı yazın..." style={inp} />
          )}
          <div style={{ fontSize:'11px', color:'#94A3B8', marginTop:'4px' }}>Raporlama için kullanılır. Boş bırakılabilir.</div>
        </div>
        ...
        {success && (
          <div style={{ background:'#EAF4EE', border:'1px solid #A7D9B8', borderRadius:'8px', padding:'10px 14px', marginBottom:'14px', fontSize:'12.5px', color:'#2E7D52', fontWeight:600 }}>
            Davet maili gönderildi! <a href="/students" style={{ color:'#1B3A6B' }}>Listeye dön →</a>
          </div>
        )}
