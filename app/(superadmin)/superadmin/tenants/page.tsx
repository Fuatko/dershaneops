...
export default function TenantsPage() {
  ...
  async function loadTenantUsers(tenantId: string) {
    const { data } = await supabase.from('profiles').select('id, full_name, role, email').eq('tenant_id', tenantId).order('role')
    setTenantUsers(data??[])
  }
  ...
  const lbl: React.CSSProperties = { display:'block', fontSize:'11px', fontWeight:600, color:'#475569', marginBottom:'5px', textTransform:'uppercase' }
  ...
  return (
    <div className="admin-page">
      ...
      <div style={{ marginBottom:'14px' }}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Kurum adı veya slug ara..." style={{ ...inp, maxWidth:'360px' }} />
      </div>

      {/* Mobil dropdown */}
      <div className="sidebar-dropdown">
        <select value={selected?.id??''} onChange={e=>{ const t=tenants.find(x=>x.id===e.target.value); if(t) selectTenant(t) }}>
        ...
        <div className="sidebar-list">
          <div style={{ background:'#fff', borderRadius:'12px', border:'1px solid #E2E8F0', overflow:'hidden' }}>
            ...
            <div style={{ maxHeight:'500px', overflowY:'auto' }}>
              {filtered.map(t=>(
                <div key={t.id} onClick={()=>selectTenant(t)} style={{ padding:'10px 14px', borderBottom:'1px solid #F8FAFC', cursor:'pointer', background:selected?.id===t.id?'#F5F8FF':'#fff', borderLeft:selected?.id===t.id?'3px solid #1B3A6B':'3px solid transparent' }}>
            ...
            <div style={{ background:'#1B3A6B', borderRadius:'12px', padding:'16px 18px', marginBottom:'12px' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'10px' }}>
                <div>
                  ...
                  <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.6)', marginTop:'2px' }}>{selected.slug} · {selected.contact_email}</div>
                </div>
                <div style={{ display:'flex', gap:'6px' }}>
                  <button onClick={()=>toggleActive(selected.id,selected.is_active)} style={{ padding:'7px 12px', borderRadius:'7px', border:'none', background:selected.is_active?'rgba(255,255,255,0.15)':'#DCFCE7', color:selected.is_active?'#fff':'#14532D', fontSize:'12px', fontWeight:600, cursor:'pointer' }}>
              ...
              {tenantUsers.length===0 ? (
                ...
                <div key={u.id} style={{ padding:'10px 16px', borderBottom:i<tenantUsers.length-1?'1px solid #F8FAFC':'none', display:'flex', alignItems:'center', gap:'10px' }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:'11px', color:'#7A8FA8' }}>{u.email}</div>
                  </div>
                  <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'8px', background:u.role==='teacher'?'#DCFCE7':u.role==='student'?'#EEF3FB':'#EDE9FE', color:u.role==='teacher'?'#14532D':u.role==='student'?'#1B3A6B':'#4C1D95' }}>
                    {u.role==='teacher'?'Öğretmen':u.role==='student'?'Öğrenci':'Veli'}
                  </span>
                </div>
              ))}
            </div>
      ...
      {showModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:'16px' }} onClick={()=>setShowModal(false)}>
          <div style={{ background:'#fff', borderRadius:'14px', padding:'24px', width:'480px', maxWidth:'100%', maxHeight:'90vh', overflowY:'auto' }} onClick={e=>e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate}>
              <div className="grid-2" style={{ marginBottom:'12px' }}>
                <div><label style={lbl}>Kurum Adı *</label><input value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))} required style={inp} /></div>
                <div><label style={lbl}>Slug *</label><input value={form.slug} onChange={e=>setForm(p=>({...p,slug:e.target.value.toLowerCase().replace(/\s/g,'-')}))} required style={inp} /></div>
              </div>
              <div style={{ marginBottom:'12px' }}><label style={lbl}>E-posta</label><input type="email" value={form.contact_email} onChange={e=>setForm(p=>({...p,contact_email:e.target.value}))} style={inp} /></div>
              <div className="grid-2" style={{ marginBottom:'12px' }}>
