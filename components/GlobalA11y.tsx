...
export default function GlobalA11y() {
  useEffect(() => {
    ...
    function addKeyboardSupport() {
      document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
          ...
          if (modal) { const btn = modal.querySelector('button') as HTMLElement; btn?.click() }
        }
      })
    }
    ...
    if (!document.getElementById('a11y-global-styles')) document.head.appendChild(styleEl)
    const timer = setTimeout(() => { enhanceButtons(); enhanceInputs(); addKeyboardSupport() }, 500)
    const observer = new MutationObserver(() => { enhanceButtons(); enhanceInputs() })
